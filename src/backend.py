...
def get_default_lists():
    ...
    routes = json.loads(open(os.path.join(os.path.dirname(
        __file__),  '../datasets/routes.json'), 'r').read())
    ...
    for k, v in routes["defaults"].items():
        ...
        for wordlist in v:
            defaults[k].extend(open(os.path.join(os.path.dirname(
                __file__), "../datasets/" + routes["lists"][k][wordlist]), "r").readlines())
...
class FakeID:
    ...
    def _email_address(self):
        ...
        username = (self.information["first_name"] +
                    "." + self.information["last_name"]).lower()
    ...
    def _height(self):
        ...
        height_ft_inches = "{}' {}\"".format(
            math.floor(height_inches / 12), height_inches % 12)
